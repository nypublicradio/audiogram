...
module.exports = function(settings) {
  ...
  // Check htpasswd auth file
  if (settings.authFile) {
    settings.authFile = normalize(settings.authFile);

    try {
      fs.accessSync(settings.authFile, fs.R_OK);
    } catch(e) {
      throw new Error("Authentication file " + settings.authFile + " does not exist or is not readable.");
    }
  }
